...
from vietcuppos.uix.sweetalert.animation import FailureAnimation, OthersAnimation, SuccessAnimation
...
class SweetAlert(MDDialog):
        ...
        if self.window_control_buttons:
            ...
            if self.window_control_buttons == "close":
                ...
                for color in data:
                    ...
                    if self.window_control_callback:
                        button.bind(
                            on_release=lambda x=button: self.window_control_callback(x))
                    button_box.add_widget(button)
    ...
    def add_buttons(self, buttons):
        if not buttons:
            ...
            button.text_color = (
                self.text_color_button if self.text_color_button else (
                    0, 0, 0, 1)
            )
    ...
    def add_icon(self, type, char="", color=()):
        ...
        if type:
            self.content_cls.add_widget(
                Widget(size_hint_y=None, height="48dp"))
        else:
            self.content_cls.padding = ("24dp", "36dp", "24dp", "24dp")

    def anim_open_dialog_pulse(self, *args):
        anim = Animation(opacity=1, _scale_x=1,
                         _scale_y=1, t="out_bounce", d=0.6)
        anim.start(self)
    ...
    def anim_close_dialog_pulse(self, *args):
        Animation(opacity=0, _scale_x=0, _scale_y=0,
                  t="out_quad", d=0.6).start(self)
